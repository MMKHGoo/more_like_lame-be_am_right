...
::: {#fig-committed-capacity .column-page-inset-right}
```{python}
#| echo: false
#| fig-cap-location: top
##| fig-caption: Built vs Remaining Committed Capacity in Honolulu
#| warning: false
...
fig = go.Figure().add_trace(
  go.Scatter(
    x=dfz.filter(pl.col('Year') < 2023)["Year"], y=dfz.filter(pl.col('Year') < 2023)["Total Committed Project Capacity As of Start year"],
    mode='lines',
  ...
  go.Scatter(
    x=dfz.filter(pl.col('Year') >= 2022)["Year"], y=dfz.filter(pl.col('Year') >= 2022)["Total Committed Project Capacity As of Start year"],
    mode='lines',
    line=dict(color='#f49300', width=3, dash='dot'),
    showlegend=False
  )
).add_trace(
  go.Scatter(
...
That is not a problem with zoning nor permitting. The zoning is there. It is a production problem[^28].

[^28]: The dashed line from 2022 to 2024 is due to DPP not releasing data for those years. Consequently added capacity is estimated from the HHFDC's annual report and includes only new RHHF, HMMF, LIHTC, and 201-H expedited projects. As none of the reviewed projects have completed construction they do not impact the built line as of yet. 
...
table_capacity = (
  ...
  # .tab_spanner(label="Total Population", columns=cs.all())
  .fmt_number(columns=cs.exclude(" "), decimals=0)
  .tab_style(
      style.text(size=16, weight="bold", ),
      loc.body(
        columns=" ",
